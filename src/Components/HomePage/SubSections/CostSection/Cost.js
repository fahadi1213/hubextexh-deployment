...
const Cost = () => {
    ...
    const totalCalPrice = ((spPrice * clPrice) - epPrice);
    // Cost Section Values Manipulations
    useEffect(() => {
        setTotalPricePerPers(totalCalPrice)
    }, [totalCalPrice])



    //Setting the State for Total Price
    const [totalPricePerPers, setTotalPricePerPers] = useState(totalCalPrice)
    ...
    //     if(pcounter == totalPricePerPers){
    //         clearInterval(myInterval)
    //     }
    //     else{
    //         pcounter++;
    //     }
    //     tpTag.current.innerHTML = `$${pcounter}`
    // }
    // const myInterval = setInterval(counterHandler,10)
    return (
        <>
                    ...
                    <Row className="mx-0">
                        <Col md={6} className="p-0 position-relative bg-white">
                            ...
                            <div className="choiceDetails position-absolute w-100 h-100 top-0 px-5 py-3 d-flex flex-column justify-content-center align-items-center" ref={cDetails}>
                                <CostCalculations Data={costData} totalPrice={totalPricePerPers} AnimFunction={offOverlay} />
                            </div>
                            <div className="cntct-form position-absolute w-100 h-100 top-0 p-5 d-flex justify-content-center align-items-center" ref={cntctMain}>
                                <div className="position-relative">
                                    <ContactFrom cntctMain={cntctMain} />
                                </div>
                            </div>
                        </Col>
